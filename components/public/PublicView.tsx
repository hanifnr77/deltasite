...
export const PublicView: React.FC<PublicViewProps> = ({ blocks, profile, socials, isPreview = false }) => {
  ...
  const renderLink = (link: LinkBlock) => {
    ...
    return (
      ...
      >
        <div 
          className={`
            relative flex items-stretch rounded-2xl transition-all duration-300 overflow-hidden min-h-[5rem]
        ...
        >
          ...
          {isImageMode ? (
            // Layout KIRI: Gambar Thumbnail
            <div className="w-28 md:w-32 shrink-0 relative overflow-hidden bg-gray-100">
               <img 
                 src={link.image} 
                 alt={link.title}
          ...
          {/* --- BAGIAN TENGAH: Teks --- */}
          <div className={`flex-1 flex flex-col justify-center min-w-0 ${isImageMode ? 'px-4 py-3' : 'px-4 py-3'}`}>
            <span className={`
            ...
            {/* Render Category Badge (Hanya muncul jika bukan image mode untuk kebersihan, atau bisa diaktifkan) */}
  ...
  const renderText = (block: TextBlock) => {
    ...
    const contentClass = `
      ...
      text-gray-700 font-heading leading-relaxed
    `;

    if (block.listType === 'number') {
       const items = block.content.split('\n').filter(i => i.trim());
       return (
         <div className={`w-full px-2 py-2 ${textAlignClass} ${contentClass}`}>
            <ol className="list-decimal list-inside space-y-1 marker:text-emerald-600 marker:font-bold">
    ...
    if (block.listType === 'bullet') {
       ...
       return (
         <div className={`w-full px-2 py-2 ${textAlignClass} ${contentClass}`}>
            <ul className="list-disc list-inside space-y-1 marker:text-emerald-600">
               {items.map((item, i) => <li key={i}>{item}</li>)}
            </ul>
         </div>
       );
      ...
      <div className={`w-full flex items-center justify-center ${heightClass}`}>
        {block.showLine && (
           <div 
              ...
              style={{ borderStyle: block.lineStyle || 'solid' }}
           ></div>
        )}
      </div>
    );
